...
class InstaZoom: NSObject {
    ...
    func gestureStateChanged (gesture: UIGestureRecognizer, zoomImageView: UIImageView) {
        ...
        if (theGesture.state == UIGestureRecognizer.State.ended || theGesture.state == UIGestureRecognizer.State.cancelled || theGesture.state == UIGestureRecognizer.State.failed) {
            self.reset()
            return;
        }
        ...
        if (!isHandlingGesture && theGesture.state == UIGestureRecognizer.State.began) {
        ...
        // Update scale & center
        if (theGesture.state == UIGestureRecognizer.State.changed) {
