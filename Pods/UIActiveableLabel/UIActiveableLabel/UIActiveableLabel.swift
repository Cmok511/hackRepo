...
public protocol UIActiveableLabelDelegate: class {
    func didSelect(_ text: String, type: UIActiveableType)
}

public typealias ConfigureLinkAttribute = (UIActiveableType, [NSAttributedStringKey : Any], Bool) -> ([NSAttributedStringKey : Any])
typealias ElementTuple = (range: NSRange, element: UIActiveableElement, type: UIActiveableType)

@IBDesignable open class UIActiveableLabel: UILabel {
    
    // MARK: - public properties
    open weak var delegate: UIActiveableLabelDelegate?

    open var enabledTypes: [UIActiveableType] = [.mention, .hashtag, .url]
    ...
    open var customColor: [UIActiveableType : UIColor] = [:] {
        didSet { updateTextStorage(parseText: false) }
    }
    open var customSelectedColor: [UIActiveableType : UIColor] = [:] {
        didSet { updateTextStorage(parseText: false) }
    }
    ...
    open func handleCustomTap(for type: UIActiveableType, handler: @escaping (String) -> ()) {
        customTapHandlers[type] = handler
    }
	
    open func removeHandle(for type: UIActiveableType) {
        switch type {
        case .hashtag:
    ...
    // MARK: - customzation
    @discardableResult
    open func customize(_ block: (_ label: UIActiveableLabel) -> ()) -> UIActiveableLabel {
        _customizing = true
        block(self)
        _customizing = false
        updateTextStorage()
    ...
    internal var customTapHandlers: [UIActiveableType : ((String) -> ())] = [:]
    ...
    lazy var activeElements = [UIActiveableType: [ElementTuple]]()
    ...
    fileprivate func updateTextStorage(parseText: Bool = true) {
        ...
        if parseText {
            clearUIActiveableElements()
            let newString = parseTextAndExtractUIActiveableElements(mutAttrString)
    ...
    fileprivate func clearUIActiveableElements() {
        selectedElement = nil
    ...
    fileprivate func parseTextAndExtractUIActiveableElements(_ attrString: NSAttributedString) -> String {
        var textString = attrString.string
        var textLength = textString.utf16.count
        var textRange = NSRange(location: 0, length: textLength)

        if enabledTypes.contains(.url) {
            let tuple = UIActiveableBuilder.URLElements(from: textString, range: textRange, maximumLenght: urlMaximumLength)
        ...
        for type in enabledTypes where type != .url {
            ...
            let hashtagElements = UIActiveableBuilder.elements(type: type, from: textString, range: textRange, filterPredicate: filter)
    ...
    //MARK: - UIActiveableLabel handler
    ...
    fileprivate func didTap(_ element: String, for type: UIActiveableType) {
...
extension UIActiveableLabel: UIGestureRecognizerDelegate {
